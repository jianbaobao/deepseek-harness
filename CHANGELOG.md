...
## 0.1.0-rc.19 (2026-08-23)
- 新增：**web 设置「统计」页**——会话数据（模型/工作区/轮数/tokens/命中率/费用）直接在 UI 展示，不用快捷键
- 新增：**web 设置「插件中心」页**——浏览 GitHub #dsh-plugin 插件，一键复制镜像加速 clone 命令，含安全/审计标记
- 桌面版与 web 端同步：桌面窗口内嵌的 web UI 即可用统计面板与插件中心；桌面端保留工具菜单
- 修复：桌面版升级覆盖式安装（版本指纹判别，归档变更强制解压，不再跳过新代码）
